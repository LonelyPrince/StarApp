...
UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UIAlertViewDelegate>
{
    ...
    NSMutableArray * getLastRecFileArr;
...
-(void) getServiceData
{
    ...
    [request setCompletionBlock:^{
        //获取 response ，看数据是否flag 和Service 等数据是否为空
        ...
        if ( data1.count == 0 && recFileData.count == 0){
            ...
            //无数据   data_valid_flag = 1   ===代表停留在语言设置界面
            ...
            }else   //无数据   data_valid_flag = 0   ===代表停留在正在搜索界面
            {
                ...
                NSLog(@"CategoryAndRECcc %@",self.CategoryAndREC);
                // 在数据为空时加载sliderview
                if (!_slideView) {
            ...
            if ([response[@"data_valid_flag"] isEqualToString:@"0"]) {
                ...
                NSLog(@"self.categorys== 22222");
                [self setCategoryAndREC:data RECFile:recFileData];
            }
            
            
            if (!_slideView) { //没有 sliderview
                //判断是不是全屏
                BOOL isFullScreen =  [USER_DEFAULT boolForKey:@"isFullScreenMode"];
                if (isFullScreen == NO) {   //竖屏状态
                    
                    NSLog(@"_slideView __ 3");
                    //设置滑动条
                    _slideView = [YLSlideView alloc];
                    _slideView = [_slideView initWithFrame:CGRectMake(0, 64.5+kZXVideoPlayerOriginalHeight+1.5,
                                                                      SCREEN_WIDTH,
                ...
                }else
                {
                    ...
                    if (!_slideView) {
                        ...
                        if (isFullScreen == NO) {   //竖屏状态
                            ...
                            NSLog(@"_slideView __ 5 %@",_slideView);
                            isHasChannleDataList = YES;
                            [self.tableForDicIndexDic removeAllObjects];
                            [USER_DEFAULT setObject:@"NO" forKey:@"NOChannelDataDefault"];
                            NSLog(@"NONO 66");
...
//    NSLog(@"indexindexindexindex %d",index);
    ...
    if (!tableForSliderView) {
        ...
        NSLog(@" tableForSliderView a 22");
        NSLog(@"tableForSliderView a 22 %@",tableForSliderView);
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        dispatch_after(popTime1, mainQueue1, ^{
            ...
            NSString * numberStr1 = [NSString stringWithFormat:@"%lu",(unsigned long)indexPathRow];
...
-(void)performChangeColor
{
    ...
    NSArray * categoryTempArr = [categorysToCategoryViewContainREC mutableCopy]; //[USER_DEFAULT objectForKey:@"categorysToCategoryViewContainREC"];
...
-(void)didselectRowToPlayClick
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([[USER_DEFAULT objectForKey:@"playStateType"] isEqualToString:mediaDisConnect]) {
            ...
            self.timer = nil;
            NSLog(@"updateProgressupdateProgress= ni11");
...
- (void)getDataService:(NSNotification *)text
{
    ...
    if (self.showTVView == YES) {
        playVideoType = 1;
        NSLog(@"XXXlive==%@",text.userInfo[@"playdata"]);
        NSLog(@"－－－－－接收到LIVe通知------");
        ...
        if (getLinkDataBool == YES) {
            ...
            //            [self.timer invalidate];
            //            self.timer = nil;
            NSLog(@"updateProgressupdateProgress= ni22");
    ...
    if (card_ret != 0) {
        ...
        if (self.showTVView == YES) {
            ...
            if (getLinkDataBool == YES) {
                ...
                [self.timer invalidate];
                self.timer = nil;
                NSLog(@"updateProgressupdateProgress= ni33");
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSLog(@"firstStart数据为cccc： %@",[[NSUserDefaults standardUserDefaults] objectForKey:@"firstStartdddd"]);
    ...
    if(![[NSUserDefaults standardUserDefaults] boolForKey:@"firstStart"]){
        ...
        NSMutableArray * judgeIsHasData =  [USER_DEFAULT objectForKey:@"serviceCategory_Default"];
        if (historyArr.count >0 || judgeIsHasData.count > 0) {
...
//                                                                                        [self tableViewCellToBlue:YLSlideTitleViewButtonTagIndex  indexhah:row AllNumberOfService:1000];
    ...
    NSNotification *notificationcc =[NSNotification notificationWithName:@"judgeIsNeedPlayNotificYes" object:nil userInfo:nil];
    //通过通知中心发送通知
    [[NSNotificationCenter defaultCenter] postNotification:notificationcc];
    NSLog(@"judgeIsNeedPlay cccacacacacacacac  1111111");
    [self showDelivaryStopped];
}
...
-(void)tableViewDataRefreshForSDTMonitor
{
    ...
    getLastCategoryArr = self.categorys;
    getLastRecFileArr = [[NSMutableArray alloc]init];
    getLastRecFileArr  = [categorysToCategoryViewContainREC mutableCopy];
    ...
    [request setCompletionBlock:^{
        ...
        if (data1.count == 0 || data1 == nil) {
            if ([response[@"data_valid_flag"] isEqualToString:@"0"]) { //正在搜索
                self.categorys = [USER_DEFAULT objectForKey:@"serviceCategory_Default"]; //data1 代表service
                NSLog(@"category %@",[USER_DEFAULT objectForKey:@"serviceCategory_Default"]);
                NSLog(@"self.categorys== 444444");
            ...
            }else if([response[@"data_valid_flag"] isEqualToString:@"1"])
            {
            ...
            NSLog(@"serviceDatanullllllllllll  3333");
            self.categorys = (NSMutableArray *)response[@"category"];  //新加，防止崩溃的地方
            NSLog(@"self.categorys== 555555");
            if (self.categorys.count > 0) {
                [USER_DEFAULT setObject:self.categorys forKey:@"serviceCategory_Default"];
                [USER_DEFAULT setObject:self.serviceData forKey:@"serviceData_Default"];
            }
        ...
        if ([self judgeIfNeedRefreshSliderView:self.categorys recFileArr:recFileData lastCategoryArr:getLastCategoryArr lastRECFileArr:getLastRecFileArr]) {
            ...
            NSLog(@"getLastRecFileArr.count %lu",(unsigned long)getLastRecFileArrTemp.count);
        ...
        for (int i = 0; i < mutableArrTemp.count; i++) {
            ...
            if (isTwoDicEqual == YES) {
                ...
                else
                {
                    ...
                    //通过通知中心发送通知
                    [[NSNotificationCenter defaultCenter] postNotification:notification1];
                    NSLog(@"judgeIsNeedPlay cccacacacacacacac  aaaaaa");
...
-(void)tableViewDataRefreshForSDTMonitorSMT
{
    ...
    [request setCompletionBlock:^{
        ...
        self.serviceData = (NSMutableArray *)data1;
        NSLog(@"serviceDatanullllllllllll  5555");
        self.categorys = (NSMutableArray *)response[@"category"];  //新加，防止崩溃的地方
        NSLog(@"self.categorys== 666666");
        if (self.categorys.count > 0) {
            [USER_DEFAULT setObject:self.categorys forKey:@"serviceCategory_Default"];
            [USER_DEFAULT setObject:self.serviceData forKey:@"serviceData_Default"];
        }
        ...
        self.tableForSliderView = [self.tableForDicIndexDic objectForKey:indexforTableToNum][1];
        NSLog(@" tableForSliderView a 55");
        NSLog(@"tableForSliderView a 55 %@",tableForSliderView);
        if (YLSlideTitleViewButtonTagIndex < self.tableForDicIndexDic.count) {
            NSNumber * numTemp = [self.tableForDicIndexDic objectForKey:indexforTableToNum][0];
...
-(void)setVideoTouchNoific
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"VideoTouchNoific_notConnect" object:nil];
    //注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(VideoTouchNoificClick_notConnect:) name:@"VideoTouchNoific_notConnect" object:nil];
...
-(void)playClick  //:(NSTimer *)timer
{
    ...
    }else if ([[USER_DEFAULT objectForKey:@"playStateType"] isEqualToString:deliveryStopTip])  {
        
    }else{
        
        NSLog(@"[USER_DEFAULT objectForKey: %@",[USER_DEFAULT objectForKey:@"playStateType"]);
        [USER_DEFAULT setObject:videoCantPlayTip forKey:@"playStateType"];
...
{
    NSLog(@"caculatorProgress 计算进度条");
    //** 计算进度条
    if(self.event_startTime.length != 0 || self.event_endTime.length != 0)
    {
        NSLog(@"caculatorProgress 计算进度条 11");
        ...
        }else
        {
            ...
            if (self.timer != nil) {
                [self.timer invalidate];
                self.timer = nil;
                NSLog(@"updateProgressupdateProgress= ni44");
                
                NSLog(@"出现一次 数据进度条数据没有清空 ");
            }
            NSLog(@"self.timer1 %@",self.timer);
            //增快速度，可以立刻显示进度条
            [NSTimer scheduledTimerWithTimeInterval:0 target:self selector:@selector(updateProgress:) userInfo:dict repeats:NO];
                    ...
                    if ([[USER_DEFAULT objectForKey:@"playStateType"] isEqualToString:mediaDisConnect]) {
                        ...
                        if ([self.categorys isKindOfClass:[NSMutableArray class]] && [epgDicToSocket isKindOfClass:[NSDictionary class]]){
                            if (epgDicToSocket.count > 14) { //录制
                                ...
                                arrForServiceByCategory = [categorysToCategoryViewContainREC mutableCopy] ; // [USER_DEFAULT objectForKey:@"categorysToCategoryViewContainREC"];
...
//从断网界面条转过来，不需要定位
-(void)VideoTouchNoificClick_notConnect: (NSNotification *)text//(NSInteger)row diction :(NSDictionary *)dic  //:(NSNotification *)text{
{
...
-(int)judgeCategoryType:(NSDictionary *)NowServiceDic
{
    ...
    NSArray * RECArrForJudgeCategory = [categorysToCategoryViewContainREC mutableCopy];
...
-(void)getServiceDataNotHaveSocket
{
    ...
    [request setCompletionBlock:^{
        ...
        self.serviceData = (NSMutableArray *)data1;
        NSLog(@"serviceDatanullllllllllll  6666");
        [self.activeView removeFromSuperview];
        self.activeView = nil;
        [self lineAndSearchBtnShow];
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(notHaveNetWork) object:nil];
...
-(void)STBDencryptNotific
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"STBDencryptNotific_noConnect" object:nil];
    //注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(popSTBAlertView_noConnect:) name:@"STBDencryptNotific_noConnect" object:nil];
...
-(void)popSTBAlertView_noConnect: (NSNotification *)text
{
...
-(void)popCAAlertView: (NSNotification *)text
{
    ...
    if (CATuner_modeStr  == [socketView.socket_ServiceModel.service_tuner_mode  intValue] &&CANetwork_idStr  == [socketView.socket_ServiceModel.service_network_id  intValue] && CATs_idStr == [socketView.socket_ServiceModel.service_ts_id intValue] && CAService_idStr == [socketView.socket_ServiceModel.service_service_id intValue]) {
        ...
        [self stopVideoPlay];
        NSLog(@"CA 触发展示CA1 %@",socketView.socket_ServiceModel.service_service_id);
        NSLog(@"CA 触发展示CA2 %d",CAService_idStr);
        //
...
-(void)tableViewDataRefreshForMjRefresh_ONEMinute
{
    ...
    [request setCompletionBlock:^{
        ...
        data1 = response[@"service"];
        if ( data1.count == 0 || data1 == nil){
            if ([response[@"data_valid_flag"] isEqualToString:@"1"]) {
                //无数据
                if (data1.count == 0 && recFileData.count == 0)
                {
                    if (_slideView) {
            ...
            }else if([response[@"data_valid_flag"] isEqualToString:@"0"]){
                //有数据
                NSMutableArray * Category_Default = [USER_DEFAULT objectForKey:@"serviceCategory_Default"];
                if (Category_Default.count == 0) {
                    return ;
                }
                self.categorys = [USER_DEFAULT objectForKey:@"serviceCategory_Default"]; //data1 代表service
                NSLog(@"self.categorys== bbbb");
                NSLog(@"category %@",[USER_DEFAULT objectForKey:@"serviceCategory_Default"]);
        ...
        if ( data1.count != 0 ){
            ...
            self.serviceData = (NSMutableArray *)data1;
            NSLog(@"serviceDatanullllllllllll  9999");
            //            self.categorys = (NSMutableArray *)response[@"category"];  //新加，防止崩溃的地方
            NSLog(@"categorys==||=One");
            ...
            self.tableForSliderView = [self.tableForDicIndexDic objectForKey:indexforTableToNum][1];
            NSLog(@" tableForSliderView a 77");
            NSLog(@"tableForSliderView a 77 %@",tableForSliderView);
            if (YLSlideTitleViewButtonTagIndex < self.tableForDicIndexDic.count) {
                NSNumber * numTemp = [self.tableForDicIndexDic objectForKey:indexforTableToNum][0];
                NSInteger index = [numTemp integerValue];
                //此处可能出现崩溃-----数组为空
...
-(void) getServiceDataForIPChange
{
    ...
    [request setCompletionBlock:^{
        ...
        self.serviceData = (NSMutableArray *)data1;
        NSLog(@"serviceDatanullllllllllll  0000");
...
{
    NSLog(@"updateProgressupdateProgress==");
    int timeCut;
    NSString *  starttime;
...
-(void)removeTopProgressView
{
    ...
    [self.timer invalidate];
    self.timer = nil;
    NSLog(@"updateProgressupdateProgress= ni55");
...
-(void)setCategoryItem:(int)inputCategoryDic
{
    ...
    NSArray * RECArrForJudgeCategory = [categorysToCategoryViewContainREC mutableCopy]; // [USER_DEFAULT objectForKey:@"categorysToCategoryViewContainREC"];
...
-(void)newSlideView
{
    ...
    }else
    {
        //判断是不是需要刷新顶部的YLSlider
        NSLog(@"准备做判断是否需要刷新！！！");
        NSArray *recFileData = [categorysToCategoryViewContainREC mutableCopy]; //[USER_DEFAULT objectForKey:@"categorysToCategoryViewContainREC"];
        NSLog(@"categorysToCategoryViewContainREC OOO  %@",categorysToCategoryViewContainREC);
        ...
        [self activeViewRemove];
        NSString * YLSlideTitleViewButtonTagIndexStr = [USER_DEFAULT objectForKey:@"YLSlideTitleViewButtonTagIndexStr"];
        NSLog(@"YLSlideTitleViewButtonTagIndexStr 8 %@",YLSlideTitleViewButtonTagIndexStr);
        NSLog(@"self.tableForDicIndexDic %@",self.tableForDicIndexDic);
        NSString *  indexforTableToNum =YLSlideTitleViewButtonTagIndexStr ;
        self.tableForSliderView = [self.tableForDicIndexDic objectForKey :indexforTableToNum] [1];
        NSLog(@" tableForSliderView a 88");
        NSLog(@"tableForSliderView a 88 %@",tableForSliderView);
        NSNumber * numTemp = [self.tableForDicIndexDic objectForKey:indexforTableToNum][0];
        ...
        NSLog(@"endRefreshing 3333");
        [self.tableForSliderView.mj_header endRefreshing];
        NSLog(@"self.tableForSliderView end= %@",self.tableForSliderView);
        NSString *urlCate = [NSString stringWithFormat:@"%@",S_category];
...
-(void)OtherDevicePushToPhoneLive:(NSNotification *)text{
    ...
    {
        NSMutableArray * liveMutableArr = [categorysToCategoryViewContainREC mutableCopy]; //[USER_DEFAULT objectForKey:@"categorysToCategoryViewContainREC"];
...
-(void)SetService_videoindex:(NSDictionary *)epgDicToSocket
{
    ...
    NSLog(@"CA 触发展示SET %@",[epgDicToSocket objectForKey:@"service_service_id"]);
...
-(void)showDelivaryStopped
{
    ...
    }else
    {
        ...
        if ([USER_DEFAULT objectForKey:@"NOChannelDataDefault"]) {
            //            [GGUtil postfullScreenBtnShow];
            //等于空时，代表没有数据，不显示按钮
        }else{
            [GGUtil postfullScreenBtnShow];
