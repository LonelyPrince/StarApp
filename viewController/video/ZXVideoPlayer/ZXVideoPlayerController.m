...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self setSubtitleRenderringOnNotific];
        [self setSubtitleRenderringOffNotific];
...
//字幕开关
-(void)setSubtitleRenderringOnNotific
{
    //此处销毁通知，防止一个通知被多次调用    // 1
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"setSubtitleRenderringOnNotific" object:nil];
    //注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setSubtitleOpen) name:@"setSubtitleRenderringOnNotific" object:nil];
}
-(void)setSubtitleRenderringOffNotific
{
    //此处销毁通知，防止一个通知被多次调用    // 1
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"setSubtitleRenderringOffNotific" object:nil];
    //注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setSubtitleOff) name:@"setSubtitleRenderringOffNotific" object:nil];
}
...
{
    //    [self setSubtitleRenderring:YES];
    [self.player setSubtitleRenderring:YES];
    NSLog(@"setSubtitleRenderring11");
}
#pragma mark - 关闭字幕
-(void)setSubtitleOff
{
    //    [self setSubtitleRenderring:NO];
    [self.player setSubtitleRenderring:NO];
...
-(void)reConnectSocketFromDisConnect // 断开连接后重新连接
{
    ...
    if (characterStr != NULL && characterStr != nil) {
        ...
        {
            //创建通知
            NSNotification *notification4 =[NSNotification notificationWithName:@"VideoTouchNoific_notConnect" object:nil userInfo:dict];
            //通过通知中心发送通知
